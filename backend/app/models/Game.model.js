...
Game.findLatest = (userID, result) => {
    sql.query("SELECT g.gameID FROM Game g, Results r WHERE r.gameID = g.gameID AND r.userID=\'" + userID + "\' AND r.result<>1 AND r.result<>-1 AND g.startDateTime >= ALL( SELECT Game.startDateTime FROM Game, Results WHERE Game.gameID = Results.gameID and userID = \'"+ userID + "\' AND result <> 1 AND result <> -1)", (err, res) => {
        if(err){
            console.log("error from db: ", err);
            result(err, null);
            return;
